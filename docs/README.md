...
- [docker_walkthrough.md](docker_walkthrough.md) — Phase 12: the full db+backend+frontend Docker Compose app. FastAPI backend + Postgres-backed approval queue/traffic log (drop-ins for Phase 6/2's interfaces, not rewrites), a no-build-step dashboard, GPU passthrough as its own verified checkpoint, and three real packaging bugs the literal container verification caught.
