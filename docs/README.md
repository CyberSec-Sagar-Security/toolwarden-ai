...
- [docker_walkthrough.md](docker_walkthrough.md) — Phase 12: the full db+backend+frontend Docker Compose app. FastAPI backend + Postgres-backed approval queue/traffic log (drop-ins for Phase 6/2's interfaces, not rewrites), a no-build-step dashboard, GPU passthrough as its own verified checkpoint, and four real bugs the literal container verification caught (three packaging, one where the "GPU-enabled" container silently never used the GPU).
- [human_testing_guide.md](human_testing_guide.md) — Phase 13 follow-up: a plain, step-by-step guide to actually running ToolWarden yourself — fresh-venv wheel install, the OpenAI demo, the MCP demo, and the full Docker stack, every command verified live against a fresh install while writing it. Scoped to what's actually confirmed working; the Anthropic adapter is documented there as built and unit-tested but not yet live-verified, with the exact steps to complete that verification once a real key is available.
- [resume_and_interview_prep.md](resume_and_interview_prep.md) — Phase 14: resume bullets and STAR-format interview talking points, pulling real numbers and findings from across all 13 prior phases rather than summarizing from memory.
