...
- Explainability (SHAP for the LightGBM layer, attention visualization for DeBERTa) was built (Phase 5) and spot-checked against real flagged examples (`docs/explainability_report.md`) — **but is not actually wired into any live decision path.** `src/toolwarden/classifier/explain.py` is never imported by `guarded_loop.py`, `mcp_proxy/server.py`, or `service/app.py`. A human reviewing a HOLD today — via the demo's console output, the MCP demo, or the Phase 12 dashboard — sees only the raw score and a templated reason string (e.g. `"score 0.513 >= hold_threshold 0.5"`), never which tokens or features drove it. The diagram above shows the intended architecture; see `docs/known_limitations.md` for this gap between design and what actually shipped, flagged during the Phase 13 audit rather than left implicit in the diagram.
